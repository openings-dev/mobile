...
describe("native profiles", () => {
  ...
  it("shows only a community's current jobs", async () => {
    ...
    expect(screen.getAllByText("openings-dev/jobs")).toHaveLength(1);
    ...
    expect(mockPush).toHaveBeenCalledWith("/authors/alice");
  });
  ...
  it("shows an author's current jobs", async () => {
    ...
    expect(mockPush).toHaveBeenCalledWith("/communities/openings-dev/jobs");
  });
});
